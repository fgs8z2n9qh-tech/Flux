...
- **Processes card** — a scrollable, **searchable** list of processes, with a
  **VRAM / CPU / RAM / NET / DISK** selector at the top that turns it into a mini
  task manager: switch between GPU memory, CPU usage (% of total), RAM (private
  working set), per-process **network** throughput, and per-process **disk/I-O**
  throughput (`\Process(*)\IO Data Bytes/sec` — all I/O like Task Manager's Disk
  column, no admin), each sorted busiest-first
